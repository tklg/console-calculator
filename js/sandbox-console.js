	...
	Model : Backbone.Model.extend({
		...
		evaluate: function(command) {
			...
			try {
				...
				if (_.isBoolean(item.result)) item._class = "boolean";
			} catch(error) {
		...
		specialCommands: function(command) {
			...
			if (command.indexOf("graph") > -1) {
				if (command.indexOf("clear") > -1) {
					...
					res = 'Showing graph'
					graph.show();
				} else if (command.indexOf("scale") > -1) {
					var sc = command.substr(command.indexOf("scale") + 6);
					res = 'Scaling graph to ' + sc;
					graph.scale(parseInt(sc));
				/*} else if (command.indexOf("bounds") > -1) {
					var bo = command.substr(command.indexOf("bounds") + 7);
					res = 'Setting graph bounds to ' + bo;
					graph.bnd(parseInt(bo));*/
				} else {
					var eval = graph.buildEquation(command.substr(command.indexOf("graph") + 6));
					graph.eval(eval);
					res = "Graphed " + eval;
				}
				return this.model.addHistory({
...
var _graph = document.getElementById("_graph");

function g(m) {
    _graph.contentWindow.postMessage(m,"http://graph.tk");
};
...
var graph = {
	...
	autoResize: function() {
		var mg = 20;
		$('#_graph').css({
			'height': $('.output').height() - (mg * 2),
			'width': $('.output').height() - (mg * 2),
			'right': mg,
			'top': mg
		});
	},
	...
	resize: function(scale) { //to make it match the autoResize because why not
		graph.scale(scale);
	},
	bnd: function(b) {
		g("bounds:" + b + ":" + b + ":" + b + ":" + b);
	},
	buildEquation: function(eval) {

		/*put stuff to build the strings like \frac{d}{dx}\left(sin\left(x\right)+log\left(x+1\right)\right) from
		d/dx(sin(x) + log(x + 1))
		...
		learn regex
		/\{\{(.+?)\}\}/g would be {{ something }}
		*/

		//none of this is needed, but whatever
		var fn = eval;
		var a = ['-1', '0'];
		eval = fn.replace('(', '\\left(')
				 .replace(')', '\\right)')
				 .replace('theta', '\\theta')
				 .replace('int', '\\int')
				 ...
				 .replace('pi', '\\pi')
				 .replace('abs', '\\left|' + a[0] + '\\right|') //get regexp or something to make this work
				 .replace('regexp of fraction', '\\frac{' + a[0] + '}{' + a[1] + '}') //this too
				 .replace('sum', '\\sum _{' + a[0] + '}^{' + a[1] + '}'); //and this
